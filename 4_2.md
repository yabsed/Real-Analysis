...
### **2. Specific Examples of Continuity (4.2.2)**
  ...
  * **(g) Thomae’s Function (Popcorn Function):**
...
### **6. Intermediate Value Theorem (IVT)**
  ...
  * **Theorem 4.2.11:** Let $f: [a, b] \to \mathbb{R}$ be continuous. If $f(a) < \gamma < f(b)$, then $\exists c \in (a, b)$ such that $f(c) = \gamma$.

    * *Proof Idea:* Let $A = \{x \in [a, b] : f(x) \le \gamma\}$. Let $c = \sup A$. Continuity ensures $f(c) = \gamma$ (by showing $f(c) < \gamma$ and $f(c) > \gamma$ lead to contradictions).

  * **Corollary 4.2.12:** The continuous image of an interval is an interval (preserves connectedness).
  * **Corollary 4.2.13 (Roots):** For every $\gamma > 0, n \in \mathbb{N}$, there exists a unique $y > 0$ such that $y^n = \gamma$.
  * **Corollary 4.2.14 (Fixed Point):** If $f: [0, 1] \to [0, 1]$ is continuous, $\exists y \in [0, 1]$ such that $f(y) = y$.

#### **Caveats & Counter-examples (4.2.15)**

  * **Converse is False:** A function can satisfy the intermediate value property but be discontinuous.
      * *Example:* $f(x) = \sin(1/x)$ for $x > 0$, $f(0) = -1$ on $[0, 2/\pi]$. Takes all values between $-1$ and $1$, but is discontinuous at 0.
  * **Requires Completeness of $\mathbb{R}$:** IVT fails on $\mathbb{Q}$.
      * *Example:* $f(x) = x^2$ on rational interval $E = [0, 2] \cap \mathbb{Q}$. $f(0) < 2 < f(2)$, but $f(x)=2$ has no solution in $E$ (since $\sqrt{2} \notin \mathbb{Q}$).
